...
│   │   ├── ui/                           # مكتبة مكونات Ark UI + Google M3 الموجهة برمجياً
│   │   │   ├── Dialog.jsx                # النوافذ المنبثقة سهلة الوصول (Ark Dialog)
│   │   │   ├── Tabs.jsx                  # التبويبات المتوافقة مع RTL ولوحة المفاتيح (Ark Tabs)
│   │   │   ├── Tooltip.jsx               # تلميحات الشاشة العائمة (Ark Tooltip)
│   │   │   ├── Switch.jsx                # مفاتيح التبديل السلسة (Ark Switch)
│   │   │   ├── Accordion.jsx             # القوائم التوسعية القابلة للطي (Ark Accordion)
│   │   │   ├── Menu.jsx                  # القوائم المنسدلة العائمة (Ark Menu)
│   │   │   ├── ark-ui.css                # تنسيقات Google Material 3 لـ Ark UI بالعربية RTL
│   │   │   └── index.js                  # التصدير المجمع لجميع المكونات
...
### 6.3 معمارية التخزين المؤقت ثنائية الطبقات (Tiered L1 Client Cache + L2 Supabase B-Tree)
- **الطبقة الأولى (L1 Hot-Cache)**:
  يعمل محرك `PatientIndexEngine` على المتصفح كذاكرة كاش فورية سريعة بحجم جلسة العمل اليومية للعيادة النشطة (Working Set من 300 إلى 2,000 مريض وموعد)، مما يوفر استجابة فورية (Zero-Latency UI) لعمليات الاستقبال وكتابة الروشتات وإدخال المرضى دون استهلاك طلبات شبكية متكررة.
- **الطبقة الثانية (L2 Backend Cloud Storage)**:
  عند البحث عن مريض غير موجود في الكاش المحلي، أو في حالة العيادات الضخمة التي تحتوي على مئات الآلاف من السجلات، يتم توجيه الاستعلام تلقائياً إلى خادم Supabase PostgreSQL المعتمد على فهرس B-Tree مركب `(clinic_id, phone)`، مع استخدام `LIMIT 1` لضمان عدم تحميل سجلات غير مطلوبة في الذاكرة.
...
### 7.1 تكامل Ark UI مع Google Material 3 (Headless Accessible Architecture)
تم دمج مكتبة **Ark UI (`@ark-ui/react`)** كمحرك قياسي وموحد للمكونات الأولية عديمة التنسيق (**Headless Accessible Primitives**)، وتتميز بالآتي:
- **إدارة الحالات الموجهة بالآلات المنطقية (State Machine Powered by Zag.js)**: كل مكون (Tabs, Dialog, Tooltip, Switch, Accordion, Menu) يُدار عبر Finite State Machine لضمان عدم حدوث تعارض في الأحداث.
- **التوافق التام مع اللغة العربية وتوجيه RTL**: تم دمج `<LocaleProvider locale="ar-EG" dir="rtl">` عند جذر التطبيق (`main.jsx`) لضمان ملاءمة مفاتيح الأسهم والتنقل الحركي تلقائياً للاتجاه من اليمين لليسار.
- **تغليف بصري كامل برموز Google M3**: تطابق تام مع الألوان والخطوط والظلال والانحناءات الكبسولية بدون أي أطر عمل خارجية ثقيلة.
- **تقسيم الحزم الذكي (Smart Chunk Splitting)**: عزل كود `@ark-ui` في ملف كاش منفصل `vendor-ark-ui.js` بحجم مضغوط لا يتعدى 3.55 kB.
...
- **الاختبارات الآلية (Automated Tests)**:
  - **29 حزمة اختبار شاملة (29/29 Test Files Passed)**.
  - **285 اختباراً بنجاح 100% (285/285 Tests Passed)** بمدة 24 ثانية، تتضمن محاكاة مليون مريض، واختبارات تكامل Ark UI، وفصل العيادات، وعمليات السداد والمحفظة.
- **بناء الإنتاج (Production Build)**:
  - بناء ناجح ومكتمل بنسبة 100% بدون أي أخطاء خلال 1.64 ثانية.
...
## 9. سجل التدقيق الجنائي والفحص العدائي (Adversarial Red-Team Audit Log — Pass 1 & Pass 2)

تم إجراء جولتي تدقيق جنائي عميق (Forensic Red-Team Audits) بواسطة عملاء التدقيق الصارم (`RuthlessCritic`) وتطبيق إصلاحات جذرية شملت:

| # | المكون المستهدف | نوع الثغرة / الخلل المكتشف | المعالجة الجذرية المطبقة |
|---|---|---|---|
| 1 | `ScheduleBuilderTab.jsx` | استدعاء فك حظر الفترات دون تمرير `clinicId` مما يهدد بتأثير عابر للمستأجرين | تمرير `clinicId` الصريح لجميع توابع `unblockSlotInDb` و `unblockFullDayInDb` |
| 2 | `Appointments.jsx` | تمرير حظر الفترات دون تحديد هوية العيادة النشطة | ربط عمليات الحظر بـ `currentClinicId` في كافة استدعاءات الخدمة |
| 3 | `DoctorAssistant.jsx` | 4 استدعاءات لإدارة الفترات المحظورة بدون هوية العيادة | إدراج `currentClinic?.id` صراحة في كافة الدوال الأربع |
| 4 | `Dashboard.jsx` | توليد معرفات المرضى والحضور المباشر بنسق `p-timestamp` مما يتعارض مع قيود UUID في Postgres | استبدال التوليد العشوائي بـ `crypto.randomUUID()` القياسي المتوافق مع قواعد البيانات |
| 5 | `walletService.js` | إمكانية إدخال مبالغ سالبة وتوليد قيود دون وسم المستأجر | تقييد المبالغ بـ `Math.abs` وإدراج `clinic_id: clinicId` في حمولة المعاملة |
| 6 | `referralService.js` | مفتاح التخزين الموضعي لسجل الإحالات لم يكن معزولاً حسب العيادة | تخصيص المفتاح ليكون `clinicflow_referrals_ledger_${clinicId}` |
| 7 | `Sidebar.jsx` | بقاء درج التنقل مفتوحاً عند التنقل على شاشات الموبايل | إضافة إغلاق تلقائي `setMobileNavOpen(false)` فور النقر على أي رابط |
| 8 | `ManageBooking.jsx` | التحقق من هوية المريض عبر تطابق اسم فرعي فضفاض يتيح تسريب بيانات المرضى | فرض مطابقة ثنائية صارمة للاسم أو الاسم الرباعي الكامل لمنع استكشاف البيانات |
| 9 | `InvoiceModal.jsx` | تشوهات الكسور العشرية للضرائب والإجماليات وقبول أسعار سالبة | فرض حد أدنى للكمية والسعر، وتطبيق تقريب عشري صارم `round2` |
| 10 | `ExpensesModal.jsx` | قبول تسجيل مبالغ سالبة للمصروفات مما يشوه حسابات الخزينة | رفض أي مبلغ `<= 0`، وتطبيق التقريب العشري، واعتماد UUID |
| 11 | `WaitingRoomQueue.jsx` | تصادم غرفة الكشف وحالة العلوق عند طلب فحص مريض آخر أثناء وجود كشف جاري | تعطيل زر "بدء الفحص" وإظهار تنبيه تحذيري فوري في حال انشغال الغرفة |
| 12 | `Sidebar.css` & `App.css` | وجود خواص فيزيائية قديمة (`right: 0`, `margin-right`) تتعارض مع RTL | استبدالها بنسبة 100% بخواص منطقية نقية (`inset-inline-start`, `margin-inline-start`) |
| 13 | `ProtectedRoute.jsx` | التحقق من صلاحيات المدير العام عبر localStorage فقط | تأكيد مطابقة البريد الإلكتروني أو التوثيق السحابي المعتمد |
